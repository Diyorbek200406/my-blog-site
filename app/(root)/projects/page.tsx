    ...
    <div className="flex flex-col items-center justify-center gap-4">
      ...
      <main className="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-2 gap-8 p-4">
        {myProjects.map((project, idx) => (
          <motion.div initial={{ opacity: 0, x: -500 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 500 }} transition={{ duration: 1, delay: 0.1 * (idx + 1), ease: "easeInOut" }} key={idx} className="max-w-sm rounded-xl overflow-hidden dark:shadow-slate-500 shadow-xl shadow-slate-400 p-4 border border-slate-200">
            <Link href={`projects/${project.name.toLowerCase()}`}>
              <Image priority className="w-full h-56 rounded-xl border border-slate-400" src={project.image} alt="Sunset in the mountains" width={100} height={100} />
            </Link>
