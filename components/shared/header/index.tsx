...
const Header = () => {
  ...
  const renderNavLinks = () => (
    <nav
      className={cn(
        "hidden z-[9] md:flex items-center gap-x-8 transition-opacity",
    ...
    >
      {navigation.map((item, idx) => (
        <Link
          ...
          href={item.route}
          className={cn(
            "text-sm font-bold py-2 dark:border-b-slate-700 border-slate-500 hover:border-b-2",
            item.route === pathname && "border-b-2"
          )}
        >
          {item.title}
        </Link>
  ...
  const renderMobileNavLinks = () => (
    <nav
      className={cn(
        "md:hidden z-[9] w-screen h-screen fixed top-0 bg-background transition-transform duration-700 delay-75",
    ...
    >
      ...
      {navigation.map((item, idx) => (
        <Link
          ...
          className={cn(
            "w-full block text-xl font-bold p-4 text-center dark:hover:bg-slate-900 hover:bg-slate-400",
            item.route === pathname && "dark:bg-slate-900 bg-slate-300"
          )}
        >
          {item.title}
        </Link>
